...
[recording](interface/metric-registry.md) functionality are separated.
...
- The [`MetricRegistry`] provides an interface which may be implemented by backends such as the
  [Prometheus Java client]
- The [`MetricFactory`] uses the [`MetricRegistry`] to create [primitive] and [derived metrics] using a [DSL]
